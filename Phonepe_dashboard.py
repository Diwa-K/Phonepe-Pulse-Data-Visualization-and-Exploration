...
# CONNECT SQL SERVER   
conn = pymysql.connect(host='localhost', user='root', password='', db='phonepe_pulse')
cursor = conn.cursor()
...
if option == 'All India':
    ...
    with tab2:
        ...
        cursor.execute(f"SELECT State, SUM(Count) FROM aggregated_user WHERE Year = '{in_us_yr}' AND Quater = '{in_us_qtr}' GROUP BY State;")
        ...
        cursor.execute(f"SELECT SUM(Count), AVG(Count) FROM aggregated_user WHERE Year = '{in_us_yr}' AND Quater = '{in_us_qtr}';")
...
# Top ten categories    
    ...
    with tab6:
        ...
        cursor.execute(f"SELECT State, SUM(RegisteredUser) AS Top_user FROM top_user WHERE Year='{top_us_yr}' GROUP BY State ORDER BY Top_user DESC LIMIT 10;")
